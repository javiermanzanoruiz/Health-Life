...
function generatePremiumPDF(jsPDFLib) {
  ...
  // Devolver el documento jsPDF. Exportar con doc.save() es más estable
  // en GitHub Pages, Safari e iPhone que crear un enlace manual con Blob.
  return doc;
}
...
function exportPDF() {
  ...
  var fileName = 'Health-Life-' + clientName.replace(/\s+/g, '-').replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ_-]/g, '') + '.pdf';

  function createAndSavePDF(Lib) {
    try {
      showToast('📄 Generando informe...');
      var doc = generatePremiumPDF(Lib);
      if (!doc) { showToast('⚠️ Error generando el PDF. Recarga e inténtalo.'); return; }
      ...
      // Fallback defensivo si doc.save no estuviera disponible.
      ...
      showToast('⚠️ Error generando el PDF. Revisa consola o recarga.');
    }
  }

  if (jsPDFLib) {
    createAndSavePDF(jsPDFLib);
    return;
  }

  showToast('📄 Cargando generador...');
  loadJsPDF(createAndSavePDF);
}
