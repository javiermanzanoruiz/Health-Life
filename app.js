...
async function generatePremiumPDF() {
  ...
  doc.setFont('times','bold'); doc.setFontSize(64); doc.setTextColor.apply(doc,coverColor);
  doc.text(String(total),105,134,{align:'center'});
  doc.setFont('helvetica','bold'); doc.setFontSize(14); doc.setTextColor.apply(doc,PDF_THEME.muted);
  doc.text('/100',105,150,{align:'center'});

  doc.setFillColor.apply(doc, coverColor);
  doc.roundedRect(74,158,62,11,5.5,5.5,'F');
  doc.setFont('helvetica','bold'); doc.setFontSize(8.5); doc.setTextColor(255,255,255);
  doc.text(status.label.toUpperCase(),105,165.5,{align:'center'});

  // Frase interpretativa bajo el score
  var coverInsight = 'El sistema presenta capacidad de mejora, pero requiere ordenar prioridades para desbloquear su potencial real.';
  if(total <= 40) coverInsight = 'El sistema muestra una desregulación importante. La prioridad es estabilizar la base antes de aumentar exigencia.';
  if(total > 60 && total <= 80) coverInsight = 'El sistema presenta una base sólida. El siguiente salto depende de afinar prioridades y reducir descompensaciones.';
  if(total > 80) coverInsight = 'El sistema presenta un nivel óptimo. La prioridad es sostener la estructura y prevenir regresiones.';
  doc.setFont('helvetica','normal'); doc.setFontSize(9.2); doc.setTextColor.apply(doc,PDF_THEME.softInk);
  doc.text(doc.splitTextToSize(coverInsight,128),105,184,{align:'center'});

  // Datos del cliente en bloque inferior limpio
  doc.setFillColor(255,255,255); doc.roundedRect(34,210,142,34,4,4,'F');
  doc.setDrawColor(232,228,216); doc.setLineWidth(0.35); doc.roundedRect(34,210,142,34,4,4,'S');
  doc.setFont('helvetica','normal'); doc.setFontSize(8.5); doc.setTextColor.apply(doc,PDF_THEME.muted);
  doc.text('Cliente',44,221); doc.text('Objetivo',44,231); doc.text('Fecha',44,241);
  doc.setFont('helvetica','bold'); doc.setFontSize(9.2); doc.setTextColor.apply(doc,PDF_THEME.ink);
  doc.text(clientName,78,221);
  doc.text(doc.splitTextToSize(clientGoal,82),78,231);
  doc.text(reportDate,78,241);
