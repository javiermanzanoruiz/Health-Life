...
async function generatePremiumPDF() {
  ...
  // Método universal: abrir en nueva pestaña funciona en todos los navegadores y dispositivos
  try {
    var blob = doc.output('blob');
    var blobUrl = URL.createObjectURL(blob);
    var newTab = window.open(blobUrl, '_blank');
    // Fallback: si el navegador bloquea la nueva pestaña, forzar descarga
    if (!newTab || newTab.closed || typeof newTab.closed === 'undefined') {
      doc.save(fileName);
    } else {
      showToast('📄 PDF listo — guárdalo desde la pestaña abierta');
...
function exportPDF() { showToast('📄 Generando informe, un momento...'); setTimeout(function(){ generatePremiumPDF(); }, 150); }
